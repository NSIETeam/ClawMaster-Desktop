...
The Tauri package is `@deepseek-ai/dsh-desktop-tauri`, independent of upstream Electron. The Host launch URL passes only in memory to a separate WebView, where upstream authentication issues the login cookie. Native capabilities match the packaged `main` and `splash` WebViews; the separate Host content and its document frames receive no native commands. The Host stays on its exact loopback origin, including port, and cannot open additional native windows. Boot logs omit the launch token. The trimmed bundle includes `native/system` and permits unused development-tool patches only in that tree; patch application failures still stop installation.

The packaged shell's CSP permits its own scripts and images, Tauri IPC, and inline styles used by progress updates. Tauri hashes packaged inline scripts; script evaluation from content, remote resources, frames, object embeds, form actions and base-URL overrides are disallowed. This policy applies to packaged shell assets; the separate Host WebView owns its web resources. [Desktop isolation](../../.agents/notes/implemented/architecture/2026-09-16-clawmaster-native-privilege-isolation.md) defines the trusted-plugin limit and verification scope.
...
[Build provenance](scripts/build-provenance.mjs) binds the complete harness build and product preparation to a full Git commit, committed tree, working-source SHA-256 and relative dirty-file list. Its [inventory](scripts/build-inventory.mjs) records component names, versions, manifests and emitted-file digests, dependency lock digests, and local patch/provenance digests. Preparation rejects source changes or replaced Host, client and frontend artifacts after compilation. The default `development` mode produces an explicit development build ID, including `dirty` when source differs. `DSH_DESKTOP_BUILD_MODE=release` requires clean source and release-mode records throughout the workflow; the release workflow also sets `DSH_DESKTOP_RELEASE_COMMIT` to its selected full commit and rejects a different checkout. Generated native icon files are verified build outputs rather than source inputs; their platform encoders can change bytes without changing the SVG. A source change requires another complete `build:harness` before `prepare:dist`.
