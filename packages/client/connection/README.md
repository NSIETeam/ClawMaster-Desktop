...
Node callers import `createWebConnectionRpc` from `@deepseek-ai/dsh-client-connection/web-rpc` and supply a fetch adapter that replaces the synthetic request origin and attaches an authenticated cookie. This ESM entry does not mount the browser plugin, discover a Host, or read credentials. The shared caller validates response envelopes and correlation ids; the caller validates the selected endpoint's result. It does not retry mutations. Cancellation is passed to fetch and does not cancel work already admitted by the Host.
