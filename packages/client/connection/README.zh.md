...
Node 调用方从 `@deepseek-ai/dsh-client-connection/web-rpc` 导入 `createWebConnectionRpc`，提供替换内部请求 origin 并附带已认证 cookie 的 fetch 适配器。这个 ESM 入口不挂载浏览器插件、不发现 Host，也不读取凭据。共用调用器校验响应信封和关联 id；调用方校验所选端点的返回值。它不重试修改请求。取消会传给 fetch，但不会取消 Host 已接收的工作。
